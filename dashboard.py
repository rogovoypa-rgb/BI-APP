...
def load_sales_data():
    try:
        ...
        possible_date_columns = [
            'Документ.Дата', 'Дата документа', 'Дата', 'Date', 
            'Документ Дата', 'ДатаДок', 'Дата заказа'
        ]
...
# ==========================================
# СТРАНИЦА 1: ПРОДАЖИ (СТАБИЛЬНАЯ ВЕРСИЯ)
# ==========================================
if page == "📈 Продажи":
    ...
    if sales_df.empty:
        ...
        # ===== ДИАГНОСТИКА (ВРЕМЕННО, ПОСЛЕ ОТЛАДКИ УДАЛИТЬ) =====
        with st.expander("🔍 Диагностика структуры данных"):
            st.write("**Колонки в sales_df:**", list(sales_df.columns))
            st.write("**has_cost_column =", 'Себестоимость' in sales_df.columns)
        ...
        required_cols = ['Выручка_без_НДС', 'Валовая_прибыль', 'Количество', 'Контрагент', 'Период.Месяц']
        missing = [c for c in required_cols if c not in df_year.columns]
        if missing:
            st.error(f"В данных отсутствуют колонки: {missing}")
            st.stop()
        
        # Определяем наличие себестоимости
        has_cost_column = 'Себестоимость' in df_year.columns
        
        # Помесячная разбивка – используем полный набор месяцев 1–12
        all_months = list(range(1, 13))
        full_months_df = pd.DataFrame({'Период.Месяц': all_months})
        
        # Агрегируем данные по месяцам для всего года
        monthly_agg = df_year.groupby('Период.Месяц').agg({
            'Выручка_без_НДС': 'sum',
            'Валовая_прибыль': 'sum',
            'Количество': 'sum'
        }).reset_index()
        if has_cost_column:
            ...
            monthly_agg = monthly_agg.merge(monthly_cost, on='Период.Месяц', how='left')
            monthly_agg['Себестоимость'] = monthly_agg['Себестоимость'].fillna(0)
        else:
            monthly_agg['Себестоимость'] = 0
        
        # Присоединяем полный набор месяцев
        monthly = full_months_df.merge(monthly_agg, on='Период.Месяц', how='left')
        monthly = monthly.fillna(0)
        monthly['Название'] = monthly['Период.Месяц'].map(month_names)
        monthly['Рентабельность'] = (monthly['Валовая_прибыль'] / monthly['Выручка_без_НДС'].replace(0, np.nan) * 100).fillna(0)
        ...
        year_revenue = monthly['Выручка_без_НДС'].sum()
        year_profit = monthly['Валовая_прибыль'].sum()
        year_margin = (year_profit / year_revenue * 100) if year_revenue > 0 else 0
        year_quantity = monthly['Количество'].sum()
        ...
        total_cost = monthly['Себестоимость'].sum()
        with total_cols[0]:
        ...
        def display_customer_analysis(customer_name, df_year, all_months, has_cost_column):
            """Отображает детальный анализ по одному контрагенту (безопасная версия)"""
            ...
            num_cols = ['Выручка_без_НДС', 'Валовая_прибыль', 'Количество', 'Себестоимость']
            for col in num_cols:
            ...
            profit = customer_monthly['Валовая_прибыль']
            ratio = profit / rev.replace(0, np.nan)
            ratio = ratio.replace([np.inf, -np.inf], np.nan).fillna(0) * 100
            customer_monthly['Рентабельность'] = ratio
            ...
            # Формируем словарь с данными, гарантируя одинаковую длину
            n_rows = len(customer_monthly)
            
            def make_col(series, fmt_func):
                lst = series.tolist()
                if len(lst) != n_rows:
                    # Обрезаем или дополняем нулями
                    lst = lst[:n_rows] if len(lst) > n_rows else lst + [0] * (n_rows - len(lst))
                return [fmt_func(x) for x in lst]
            
            table_dict = {
                'Месяц': make_col(customer_monthly['Название'], lambda x: str(x) if pd.notna(x) else ''),
                'Выручка': make_col(customer_monthly['Выручка_без_НДС'], lambda x: f"{format_number(x)} ₽"),
                'Прибыль': make_col(customer_monthly['Валовая_прибыль'], lambda x: f"{format_number(x)} ₽"),
                'Рентабельность': make_col(customer_monthly['Рентабельность'], lambda x: f"{format_float(x, 1)}%"),
                'Кол-во (шт)': make_col(customer_monthly['Количество'], format_number),
            }
            if has_cost_column and 'Себестоимость' in customer_monthly.columns:
                table_dict['Себестоимость'] = make_col(
                    customer_monthly['Себестоимость'],
                    lambda x: f"{format_number(x)} ₽"
                )
            
            display_df = pd.DataFrame(table_dict)
        ...
        # Вывод анализа для каждого из ТОП-5
        for customer in top5_revenue:
            display_customer_analysis(customer, df_year, all_months, has_cost_column)
        
        # ===== СВОДНАЯ ТАБЛИЦА ПО ТОП-5 =====
        ...
        if summary_top5:
            summary_df = pd.DataFrame(summary_top5).sort_values('Выручка', ascending=False)
            display_summary = summary_df.copy()
            for col in ['Выручка', 'Прибыль', 'Себестоимость']:
                display_summary[col] = display_summary[col].apply(lambda x: f"{format_number(x)} ₽")
...
# ==========================================
    ...
    if logistics_df.empty:
        ...
        # Количество строк на одну отгрузку
...
# ==========================================
    ...
    if sales_df.empty:
        ...
        **Формула:** Себестоимость без НДС / Количество
        ...
        unique_groups = sorted(df_cost[group_col].dropna().unique())
        if len(unique_groups) == 0:
            st.warning("Нет данных для отображения")
        else:
            col_select1, col_select2 = st.columns([2, 1])
            with col_select1:
                selected_group = st.selectbox("📂 Выберите группу номенклатур", unique_groups, help=f"Группировка по столбцу '{group_col}'")
            with col_select2:
                show_iqr_analysis = st.checkbox("📊 Показать IQR-анализ выбросов", value=True, help="Метод межквартильного расстояния для обнаружения выбросов")
            ...
            if df_group.empty:
                ...
                with col_filter1:
                    ...
                    period_filter = st.selectbox("📅 Период", period_options, index=default_index)
                with col_filter2:
                    sort_options = [
                    ...
                    sort_by = st.selectbox("📊 Сортировка графиков", sort_options, index=default_sort_index)
                ...
                for nomen in nomenclatures:
                    ...
                    if not df_nomen.empty:
                        monthly_data = df_nomen.groupby('Период')['Себестоимость_единицы'].mean().sort_index()
                ...
                if show_iqr_analysis and len(nomen_stats) > 0:
                    ...
                    if all_costs:
                        ...
                        if iqr_all['q1'] is not None:
                            ...
                            fig_box = go.Figure()
                            fig_box.add_trace(go.Box(y=all_costs, name=selected_group, boxmean='sd', marker_color='#2E86AB', line_color='#1A5276'))
                            fig_box.update_layout(title=f'Распределение себестоимости в группе "{selected_group}"', yaxis_title='Себестоимость (₽/ед.)', height=400, showlegend=False)
                            ...
                            if len(iqr_all['outliers']) > 0:
                                ...
                                if outlier_nomenclatures:
                                    outlier_df = pd.DataFrame(outlier_nomenclatures).sort_values('Количество_выбросов', ascending=False)
                ...
                for idx, stat in enumerate(nomen_stats):
                    ...
                    with col_left:
                        ...
                        st.markdown(f"""
                        <div style='background-color: #F0F2F6; border-radius: 10px; padding: 12px; margin-top: 30px;'>
                    ...
                    with col_right:
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=periods, y=costs, mode='lines+markers', line=dict(width=2, color='#2E86AB'), marker=dict(size=8, color='#1A5276'), name='Себестоимость', hovertemplate='%{x}<br>Себестоимость: %{y:.2f} ₽/ед.<extra></extra>'))
                        if show_iqr_analysis and stat['IQR_анализ']['q1'] is not None:
                            iqr_res = stat['IQR_анализ']
                            if iqr_res['lower_bound'] is not None:
                                fig.add_hline(y=iqr_res['lower_bound'], line_dash="dash", line_color="orange", annotation_text=f"Нижняя граница: {format_number(iqr_res['lower_bound'])}", annotation_position="bottom left")
                            if iqr_res['upper_bound'] is not None:
                                fig.add_hline(y=iqr_res['upper_bound'], line_dash="dash", line_color="orange", annotation_text=f"Верхняя граница: {format_number(iqr_res['upper_bound'])}", annotation_position="top left")
                            if iqr_res['outliers']:
                                outlier_periods = []
                                outlier_y = []
                                ...
                                if outlier_periods and outlier_y:
                                    fig.add_trace(go.Scatter(x=outlier_periods, y=outlier_y, mode='markers', marker=dict(size=15, color='red', symbol='x'), name='Выбросы (IQR)', hovertemplate='%{x}<br>⚠️ ВЫБРОС: %{y:.2f} ₽/ед.<extra></extra>'))
                        fig.add_hline(y=stat['Средняя'], line_dash="dash", line_color="red", annotation_text=f"Среднее: {format_number(stat['Средняя'])} ₽/ед.", annotation_position="top right")
                        if stat['Стд_отклонение'] > 0:
                            fig.add_hrect(y0=stat['Средняя'] - stat['Стд_отклонение'], y1=stat['Средняя'] + stat['Стд_отклонение'], fillcolor="rgba(46, 134, 171, 0.2)", line_width=0, annotation_text="±1σ", annotation_position="bottom right")
                        fig.update_layout(title="", xaxis_title="Период (месяц/год)", yaxis_title="Себестоимость без НДС на единицу (₽/ед.)", hovermode='x unified', height=350, margin=dict(l=40, r=40, t=30, b=40))
                    ...
                    if show_iqr_analysis and stat['Количество_периодов'] >= 4 and stat['IQR_анализ']['q1'] is not None:
                        with st.expander(f"📊 Boxplot для {nomen} (IQR-анализ)"):
                            ...
                            fig_box_nomen.add_trace(go.Box(y=costs, name=nomen, boxmean='sd', marker_color='#2E86AB', line_color='#1A5276'))
                            if stat['IQR_анализ']['outliers']:
                                iqr_res = stat['IQR_анализ']
                                fig_box_nomen.add_trace(go.Scatter(y=iqr_res['outliers'], mode='markers', marker=dict(size=10, color='red', symbol='x'), name='Выбросы'))
                            fig_box_nomen.update_layout(title=f'Распределение себестоимости для "{nomen}"', yaxis_title='Себестоимость (₽/ед.)', height=300, showlegend=True)
                ...
                if len(nomen_stats) > 0:
                    ...
                    if not summary_df.empty:
                        ...
                        st.dataframe(display_summary, use_container_width=True, hide_index=True, column_config={"Выбросов (IQR)": st.column_config.NumberColumn("Выбросов (IQR)", help="Количество выбросов по методу IQR")})
                        ...
                        st.download_button("📥 Скачать сводную таблицу (CSV)", csv_data, f"cost_per_unit_summary_{selected_group}_{pd.Timestamp.now().strftime('%Y%m%d')}.csv", "text/csv")
                ...
                if len(nomen_stats) > 1 and 'Подкатегория' in df_cost_filtered.columns:
                    ...
                    category_analysis = category_analysis.rename(columns={'Подкатегория': 'Группа', 'mean': 'Средняя', 'std': 'Стд_отклонение', 'min': 'Минимум', 'max': 'Максимум', 'count': 'Количество_записей'})
                    if not category_analysis.empty:
                        ...
                        fig_compare.add_trace(go.Bar(x=category_analysis['Группа'], y=category_analysis['Средняя'], marker_color='#2E86AB', error_y=dict(type='data', array=category_analysis['Стд_отклонение'], visible=True), name='Средняя ± отклонение'))
                        fig_compare.update_layout(title='Средняя себестоимость по подкатегориям', xaxis_title='Подкатегория', yaxis_title='Себестоимость (₽/ед.)', height=400)
...
# ==========================================
    ...
    if production_df.empty:
        ...
        if batches.empty:
            ...
            with col_filter2:
                sort_by = st.selectbox("📊 Сортировка партий", ["По номеру (старые сверху)", "По номеру (новые сверху)", "По себестоимости (от низкой)", "По себестоимости (от высокой)"])
            st.divider()
            ...
            if len(batches) >= 2:
                ...
                with col_left:
                    st.markdown(f"""
                    <div style='background-color: #D4EDDA; border-radius: 10px; padding: 15px; text-align: center; border: 1px solid #28A745;'>
                ...
                with col_right:
                    st.markdown(f"""
                    <div style='background-color: #F8D7DA; border-radius: 10px; padding: 15px; text-align: center; border: 1px solid #DC3545;'>
                ...
                st.markdown(f"""
                <div style='background-color: #E2E3E5; border-radius: 10px; padding: 10px; text-align: center; margin-top: 10px;'>
            ...
            fig_cost = go.Figure()
            fig_cost.add_trace(go.Scatter(x=batches_sorted['Партия_стр'], y=batches_sorted['Себестоимость_единицы'], mode='lines+markers', name='Себестоимость единицы', line=dict(color='#2E86AB', width=2), marker=dict(size=8, color='#1A5276')))
            fig_cost.add_hline(y=avg_cost, line_dash="dash", line_color="red", annotation_text=f"Среднее: {format_number(avg_cost)} ₽/шт.", annotation_position="top right")
            fig_cost.update_layout(title="Динамика себестоимости единицы продукции по партиям", xaxis_title="Партия", yaxis_title="Себестоимость (₽/шт.)", hovermode='x unified', height=450, xaxis_tickangle=-45)
            ...
            if selected_batch == 'Все партии':
                ...
                fig_hist = px.histogram(batches, x='Себестоимость_единицы', nbins=20, title='Гистограмма распределения себестоимости', labels={'Себестоимость_единицы': 'Себестоимость (₽/шт.)'})
                ...
                if batch_info.empty:
                    ...
                    if not batch_materials.empty:
                        ...
                        fig_pie = px.pie(batch_materials, values='Себестоимость_на_единицу_продукции', names='Сырье', title=f'Распределение затрат в партии {selected_batch}', labels={'Себестоимость_на_единицу_продукции': 'Затраты (₽/шт.)'})
                        st.plotly_chart(fig_pie, use_container_width=True)
                        st.subheader("📊 Анализ сырья")
                        col1, col2 = st.columns(2)
                        with col1:
                            fig_bar = px.bar(batch_materials, x='Сырье', y='Цена_сырья', title='Цена сырья', labels={'Цена_сырья': 'Цена (₽)', 'Сырье': ''})
                            fig_bar.update_layout(xaxis_tickangle=-45)
                            st.plotly_chart(fig_bar, use_container_width=True)
                        with col2:
                            fig_qty = px.bar(batch_materials, x='Сырье', y='Количество_сырья', title='Количество использованного сырья', labels={'Количество_сырья': 'Количество', 'Сырье': ''})
        ...
        if 'batches' in locals() and not batches.empty and len(batches) >= 2:
            ...
            if batch1 and batch2 and batch1 != batch2:
                ...
                with c1:
                    ...
                    st.metric("Разница в себестоимости", f"{format_number(diff_cost)} ₽/шт.", delta=f"{format_float(diff_cost / batch1_info['Себестоимость_единицы'] * 100, 1)}%")
                with c2:
                ...
                if not materials1.empty and not materials2.empty:
                    ...
                    comparison = materials1[['Сырье', 'Себестоимость_на_единицу_продукции']].merge(materials2[['Сырье', 'Себестоимость_на_единицу_продукции']], on='Сырье', how='outer', suffixes=('_1', '_2')).fillna(0)
                    ...
                    fig_comp.update_layout(title='Сравнение затрат на сырьё по партиям', xaxis_title='Сырьё', yaxis_title='Затраты (₽/шт.)', barmode='group', xaxis_tickangle=-45)
                    st.plotly_chart(fig_comp, use_container_width=True)

# ==========================================
# СТРАНИЦА 5: ЛОГИСТИКА UPDATE (из logistics_data.xlsx, два уровня анализа)
# ==========================================
    ...
    if logistics_df.empty:
        ...
        if df_l1_orders.empty:
            ...
            monthly_l1 = df_l1_orders.groupby('Месяц_название').agg({
                ...
                'Кол-во паллет': 'sum',
                'Кол-во_позиций': 'sum',
                'Город': lambda x: list(set([item for sublist in x for item in sublist]))
            }).reset_index()
            monthly_l1 = monthly_l1.rename(columns={'Дата': 'Количество_заказов', 'Город': 'Уникальные_города'})
            ...
            st.dataframe(display_l1[['Месяц_название', 'Количество_заказов', 'Кол-во паллет', 'Кол-во_уникальных_городов',
            ...
            with col_g1:
                fig1 = go.Figure()
                fig1.add_trace(go.Bar(x=monthly_l1['Месяц_название'], y=monthly_l1['Стоимость_товара'], name='Стоимость товаров', marker_color='#2E86AB', text=monthly_l1['Стоимость_товара'].apply(lambda x: f'{format_number(x)} ₽'), textposition='outside'))
                fig1.add_trace(go.Bar(x=monthly_l1['Месяц_название'], y=monthly_l1['Логистика_затраты'], name='Затраты на логистику', marker_color='#D9534F', text=monthly_l1['Логистика_затраты'].apply(lambda x: f'{format_number(x)} ₽'), textposition='outside'))
                fig1.update_layout(title='Стоимость товаров vs Затраты на логистику по месяцам', xaxis_title='Месяц', yaxis_title='Сумма (₽)', barmode='group', height=400)
                st.plotly_chart(fig1, use_container_width=True)
            with col_g2:
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=monthly_l1['Месяц_название'], y=monthly_l1['Кол-во_уникальных_городов'], mode='lines+markers', name='Уникальные города', line=dict(color='#52B788', width=3), marker=dict(size=10, color='#2D6A4F')))
                fig2.add_trace(go.Scatter(x=monthly_l1['Месяц_название'], y=monthly_l1['Количество_заказов'], mode='lines+markers', name='Количество заказов', line=dict(color='#FFB74D', width=2, dash='dash'), marker=dict(size=8, color='#F57C00'), yaxis='y2'))
                fig2.update_layout(title='Динамика уникальных городов и количества заказов', xaxis_title='Месяц', yaxis_title='Количество уникальных городов', yaxis2=dict(title='Количество заказов', overlaying='y', side='right'), height=400, hovermode='x unified')
                st.plotly_chart(fig2, use_container_width=True)
            fig3 = go.Figure()
            fig3.add_trace(go.Bar(x=monthly_l1['Месяц_название'], y=monthly_l1['Доля_логистики_%'], name='Доля логистики', marker_color='#7B68EE', text=monthly_l1['Доля_логистики_%'].apply(lambda x: f'{format_float(x, 1)}%'), textposition='outside'))
            fig3.update_layout(title='Доля логистики в стоимости товаров по месяцам', xaxis_title='Месяц', yaxis_title='Доля (%)', height=400)
            st.plotly_chart(fig3, use_container_width=True)
            st.divider()
        
        # ===== УРОВЕНЬ 2: АНАЛИЗ ПО ЗАКАЗАМ =====
        ...
        if df_l2.empty:
            ...
            display_l2['Номенклатуры'] = display_l2['Список_номенклатур'].apply(lambda x: ', '.join(x[:3]) + ('...' if len(x) > 3 else ''))
            st.dataframe(display_l2[['Дата_отгрузки', 'Города', 'Кол-во паллет', 'Кол-во_позиций', 'Кол-во_городов_в_заказе',
            ...
            with st.expander("🏙️ Детализация по городам в заказах"):
                ...
                st.dataframe(display_cities[['Дата_отгрузки', 'Город', 'Кол-во паллет_по_городу', 
            ...
            with col_export1:
                ...
                csv_orders = export_orders[['Дата', 'Города', 'Кол-во паллет', 'Кол-во_позиций', 
                    'Стоимость_товара', 'Логистика_затраты']].to_csv(index=False, sep=';', decimal=',').encode('utf-8-sig')
                st.download_button("📥 Скачать детализацию по заказам (CSV)", csv_orders, f"logistics_orders_{selected_year_l2}_{selected_month_l2}.csv", "text/csv", use_container_width=True)
            with col_export2:
                order_dates = df_l2['Дата'].tolist()
                ...
                csv_cities = df_city_export[['Дата', 'Город', 'Кол-во паллет_по_городу', 
                    'Стоимость_товара_по_городу', 'Логистика_затраты_по_городу']].to_csv(index=False, sep=';', decimal=',').encode('utf-8-sig')
                st.download_button("📥 Скачать детализацию по городам (CSV)", csv_cities, f"logistics_cities_{selected_year_l2}_{selected_month_l2}.csv", "text/csv", use_container_width=True)
            
            st.subheader("📊 ВИЗУАЛИЗАЦИЯ ПО ЗАКАЗАМ")
            col_g3, col_g4 = st.columns(2)
            with col_g3:
                top_orders = df_l2.nlargest(10, 'Логистика_затраты')
                fig4 = px.bar(top_orders, x='Дата', y='Логистика_затраты', title='Топ-10 заказов по затратам на логистику',
                    labels={'Дата': 'Дата отгрузки', 'Логистика_затраты': 'Затраты (₽)'},
                    color='Кол-во_городов_в_заказе', color_continuous_scale='Viridis',
            ...
            with col_g4:
                city_stats = df_city_detail_filtered.groupby('Город').agg({
                    ...
                    'Логистика_затраты_по_городу': 'sum',
                    'Стоимость_товара_по_городу': 'sum'
                }).reset_index().sort_values('Логистика_затраты_по_городу', ascending=False)
                fig5 = px.bar(city_stats, x='Город', y='Логистика_затраты_по_городу', title='Затраты на логистику по городам',
                    labels={'Логистика_затраты_по_городу': 'Затраты (₽)', 'Город': ''},
                    color='Кол-во паллет_по_городу', color_continuous_scale='Blues',
                    text=city_stats['Логистика_затраты_по_городу'].apply(lambda x: format_number(x)))
                fig5.update_layout(height=400)
                st.plotly_chart(fig5, use_container_width=True)
            fig6 = go.Figure()
            fig6.add_trace(go.Scatter(x=df_l2['Стоимость_товара'], y=df_l2['Логистика_затраты'], mode='markers',
                marker=dict(size=df_l2['Кол-во_городов_в_заказе']*8+5, color=df_l2['Кол-во_городов_в_заказе'], colorscale='Viridis', showscale=True, colorbar=dict(title='Кол-во городов'), sizemode='diameter'),
                text=df_l2['Дата'].dt.strftime('%d.%m.%Y') + ' - ' + df_l2['Город'].apply(lambda x: ', '.join(set(x))), hoverinfo='text+x+y'))
            fig6.update_layout(title='Стоимость товаров vs Затраты на логистику по заказам', xaxis_title='Стоимость товаров (₽)', yaxis_title='Затраты на логистику (₽)', height=450)
