...
# ==========================================
# СТРАНИЦА 1: ПРОДАЖИ (ПОЛНАЯ ВЕРСИЯ С ИСПРАВЛЕННОЙ СЕБЕСТОИМОСТЬЮ)
# ==========================================
if page == "📈 Продажи":
    ...
    if sales_df.empty:
        ...
        required_cols = ['Выручка_без_НДС', 'Валовая_прибыль', 'Количество', 'Период.Месяц', 'Контрагент']
        ...
        if has_cost_column:
            ...
            monthly = monthly.merge(monthly_cost, on='Период.Месяц', how='left')
            monthly['Себестоимость'] = monthly['Себестоимость'].fillna(0)
        else:
            monthly['Себестоимость'] = 0
        
        # Приводим к полному набору месяцев
        full_months_df = pd.DataFrame({'Период.Месяц': all_months})
        monthly = full_months_df.merge(monthly, on='Период.Месяц', how='left')
        ...
        def display_customer_analysis(customer_name, df_year, all_months, has_cost_column):
            """Отображает детальный анализ по одному контрагенту (исправленная версия)"""
