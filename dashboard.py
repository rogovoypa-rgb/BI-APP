...
def load_sales_data():
    ...
    # ВАЖНО: Используем столбец 'Себестоимость без НДС' (индекс 19) - это себестоимость БЕЗ НДС
    df['Себестоимость_без_НДС'] = pd.to_numeric(df['Себестоимость без НДС'], errors='coerce').fillna(0)
    
    # Расчёт прибыли и рентабельности
    # Выручка без НДС - Себестоимость без НДС = Валовая прибыль (без НДС)
    ...
    df = df.rename(columns={
        ...
        'Себестоимость без НДС': 'Себестоимость',  # ← переименовываем для удобства
